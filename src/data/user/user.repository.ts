...
export default class UserRepository extends BaseRepository {
  ...
  constructor() {
    super('users');
    const _new = new User();
    this._with = [{ path: 'photo' }, { path: 'credential' }];
  ...
  public logout = async (body: any) => {
    try {
      ...
      await this.removeDevice(body);
      user.devices = user.devices.filter(function(ele) {
        return ele != body.device;
      });
      return user;
    } catch (error) {
      throw new Error(error);
  ...
  public addDevice = async (body: any) => {
    try {
      const user = await this._model.findOne({
        email: body.email.toLowerCase()
      });

      if (!user.devices.includes(body.device)) {
        user.devices.push(body.device);
      }

      user.devices = user.devices.filter((x: any) => x != undefined);
  ...
  public removeDevice = async (body: any) => {
    try {
      const user = await this._model.findOne({
        email: body.email.toLowerCase()
      });
      ...
      if (user.devices.includes(body.device)) {
        user.devices = user.devices.filter((ele: any) => ele != body.device);
      }
  ...
  public changePassword = async (user: any, body: any) => {
    try {
      const result = await this.findByIdAndUpdate(user.id, {
        password: bcrypt.hashSync(body.new_password, bcrypt.genSaltSync(10))
      });
      return result;
    } catch (error) {
      throw new Error(error);
  ...
  public getFullUser = async (_id: string) => {
    try {
      const user = await this._model
        ...
        .populate({ path: 'pending_request' })
        .populate({
          path: 'favorites',
          populate: { path: 'poi', populate: { path: 'images' } }
        });
      if (!user) throw new Error('No User Found');
      return user;
    } catch (error) {
    ...
    try {
      const user = await this._model
        ...
        .populate({
          ...
          populate: {
            path: 'poi',
            populate: { path: 'city', populate: { path: 'way' } }
          }
        });
      if (!user) throw new Error('No User Found');
      return user;
